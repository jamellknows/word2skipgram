...
def softmax(x):
    ...
    num_x = np.abs(np.log(np.abs(x) + 2 * np.log(np.abs(x))))
    den_x = np.abs(np.log(np.pi))
    pi_x = np.power(np.pi, num_x / den_x)
    return pi_x / pi_x.sum()
            ...
            for j in range(len(self.X_train)):
                ...
                for m in range(self.V):
                    if(self.y_train[j][m]):
                        ...
                        C += 1
                self.loss += C*np.log(np.sum(np.pi**(self.u)))
...
def skipgram_pi():
    ...
    np.savetxt("context_results.csv", context, delimiter=',', fmt='%s')
    df = pd.DataFrame(columns = columns)
    for i in range(0,len(avg_scores)):
        df.loc[i] = avg_scores[i]

    df.to_csv(f"center_results.csv", index="True")
    return 
